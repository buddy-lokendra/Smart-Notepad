...
from speech_recognition import RequestError, UnknownValueError, WaitTimeoutError
...
class Notepad:
    def __init__(self):
        ...
        activeTimeThread = threading.Thread(target=self.activetimefun)
        activeTimeThread.start()
        CurrentDateTimeThread = threading.Thread(target=self.currentdatetimefun)
        CurrentDateTimeThread.start()
    ...
    def set_tool_bar(self):
        ...
        self.currentdatetime = ttk.Label(self.tool_bar)
        self.currentdatetime.grid(row=0, column=10, padx=5)

        self.activetime = ttk.Label(self.tool_bar)
        self.activetime.grid(row=0, column=11, padx=5)
    ...
    def set_file_sub_menu(self):
        ...
        self.file.add_command(label='Save As', image=self.new_icon, compound=tk.LEFT, accelerator='Alt+S',command=self.save_as)
        self.file.add_command(label='Exit', image=self.exit_icon, compound=tk.LEFT, accelerator='Ctrl+Q',command=self.exit_func)
        self.p_file.add_command(label='Secure Files', compound=tk.LEFT, accelerator='Ctrl+P',command=self.secure_file)

    def set_edit_sub_menu(self):
        self.edit.add_command(label='Copy', image=self.copy_icon, compound=tk.LEFT, accelerator='Ctrl+C',command=lambda: self.text_editor.event_generate("<Control c>"))
        self.edit.add_command(label='Paste', image=self.paste_icon, compound=tk.LEFT, accelerator='Ctrl+V',command=lambda: self.text_editor.event_generate("<Control v>"))
        self.edit.add_command(label='Cut', image=self.cut_icon, compound=tk.LEFT, accelerator='Ctrl+X',command=lambda: self.text_editor.event_generate("<Control x>"))
        self.edit.add_command(label='Clear All', image=self.clear_all_icon, compound=tk.LEFT, accelerator='Alt+X',command=lambda: self.text_editor.delete(1.0, tk.END))
        self.edit.add_command(label='Find', image=self.find_icon, compound=tk.LEFT, accelerator='Ctrl+F',command=self.find_func)

    def set_view_sub_menu(self):
        self.view.add_checkbutton(label='Tool Bar', onvalue=True, offvalue=0, variable=self.show_toolbar,image=self.tool_bar_icon,compound=tk.LEFT, command=self.hide_toolbar)
        self.view.add_checkbutton(label='Status Bar', onvalue=1, offvalue=False, variable=self.show_statusbar,image=self.status_bar_icon, compound=tk.LEFT, command=self.hide_statusbar)
    ...
    def open_file(self, event=None):
        try:
            ...
            if self.db_controller.if_secure_file(self.base):
    ...
    def open_secure_file(self):
        ...
        if self.db_controller.if_secure_file(base):
            pwd = self.get_file_pwd()
    ...
    def activetimefun(self):
        global h, m, s, task
        texts ='Active Time  :  00:00:00'
        self.activetime.configure(text=texts)
        while task:
            time.sleep(1)
            s += 1
            if s >= 60:
            ...
            if len(str(h)) == 1:
                ...
                hor = str(h)
            texts = f'Active Time  :  {hou}:{min}:{sec}'
            self.activetime.configure(text=texts)


    def currentdatetimefun(self):
        global task
        while task:
            obj = datetime.now()
            str = obj.strftime('Date :  %x    Time  :  %X')
            self.currentdatetime.config(text=str)
            time.sleep(1)
